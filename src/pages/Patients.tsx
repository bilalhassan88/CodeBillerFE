...
export default function Patients() {
  ...
  const handleAdd = async (e: React.FormEvent) => {
    ...
    setSubmitError(null);
    if (!clinicId) {
      setSubmitError(Messages.notAuthenticatedWithClinic);
      return;
    }
      ...
      <div className="card mt-8">
        ...
        {loading ? (
          ...
          <>
            ...
            {items.length === 0 && (
              <p className="py-12 text-center text-slate-500 dark:text-slate-400">{Messages.emptyPatients}</p>
            )}
            {totalCount > 0 && (
              <div className="mt-4 flex items-center justify-between border-t border-slate-200 pt-4 dark:border-slate-600">
                <p className="text-sm text-slate-500 dark:text-slate-400">{Messages.paginationTotal(totalCount)}</p>
                <div className="flex gap-2">
                  <button type="button" className="btn-secondary" disabled={page <= 1} onClick={() => setPage(Math.max(1, page - 1))}>{Messages.paginationPrevious}</button>
                  <span className="self-center text-sm text-slate-600 dark:text-slate-300">{Messages.paginationPage(page, totalPages)}</span>
                  <button type="button" className="btn-secondary" disabled={page >= totalPages} onClick={() => setPage(Math.min(totalPages, page + 1))}>{Messages.paginationNext}</button>
                </div>
              </div>
            )}
          </>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
          <div className="w-full max-w-md rounded-2xl bg-white p-6 shadow-xl dark:bg-dark-card dark:border dark:border-slate-600">
            <div className="flex items-center justify-between border-b border-slate-200 pb-4 dark:border-slate-600">
              <h2 className="text-lg font-semibold text-slate-900 dark:text-slate-100">{Messages.actionAddPatient}</h2>
